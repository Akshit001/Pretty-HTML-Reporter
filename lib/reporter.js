...
;
                    ...
                    outputFile = outputDir +  '/Reports.html';
                    writeSingleResults( outputFile, template);
...
var i =0;
    ...
    function writeSingleResults(outputFile, template) {
        pendingFileWritings++;
 i++;
        helper.mkdirIfNotExists(path.dirname(outputFile), function () {
            ...
            if (!!template) {
                ...
                var formattedResults = formatResults(browsers[i]);

                template = searchReplace(template, browsers[i], i);
                template = template.replace('Test_Results'+i, formattedResults.toString({ pretty: true }));
    ...
    function searchReplace(template, browser, i) {
        var counts;
        ...
        template = template.replace('Auto_Reload'+i, autoReload);
        template = template.replace('Refresh_Timeout'+i, refreshTimeout);
        template = template.replace('DateTime_Stamp'+i, formatTimestamp());
        template = template.replace('Total_Test_Count'+i, counts.totalCnt);
        template = template.replace('Total_Pass_Count'+i, counts.passCnt);
        template = template.replace('Total_Skip_Count'+i, counts.skipCnt);
        template = template.replace('Total_Fail_Count'+i, counts.failCnt);
